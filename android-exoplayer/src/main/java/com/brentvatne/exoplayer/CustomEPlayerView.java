...
import com.daasuu.epf.CustomEPlayerRenderer;
...
public class CustomEPlayerView extends GLSurfaceView implements VideoListener {

    private CustomEPlayerRenderer renderer;
    private SimpleExoPlayer player;

    private float videoAspect = 1f;
    private PlayerScaleType playerScaleType = PlayerScaleType.RESIZE_FIT_WIDTH;

    public CustomEPlayerView(Context context) {
        this(context, null);
    }

    public CustomEPlayerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        setEGLContextFactory(new EContextFactory());
        setEGLConfigChooser(new EConfigChooser());
    }

    public void setup(final int angle) {
        renderer = new CustomEPlayerRenderer(this);
        renderer.setAngle(angle);
        setRenderer(renderer);
    }

    public CustomEPlayerView setSimpleExoPlayer(SimpleExoPlayer player) {
        if (this.player != null) {
            this.player.release();
            this.player = null;
        }
